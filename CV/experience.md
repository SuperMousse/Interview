...
b. MF(矩阵分解): 现有M = (user, item)的稀疏矩阵, 表示用户对于某些商品的评价分数, 希望预测出某个用户对于某一个未知的商品的评价分数, 希望进行如下矩阵分解: M = U I^T, U = (user, embedding), I = (item, embedding), 所以我们求解Loss = sum(m_{ij} - u^T * i), 求导通过梯度下降更新u, i, 实际使用时u, i也可以加入l2正则化
