...
ROC: 横轴FPR=FP/(FP+TN), 纵轴TPR=TP/(TP+FN), FPR只跟负样本有关, TPF只跟正样本有关, 正负样本之间没有交叉计算, 改变正负样本的比例并不会改变模型对于正负样本的判断, 因此ROC会保持稳定, 也就是说负采样并不会影响ROC, AUC 
...
h. SVM高斯核相当于把特征映射到多少维?
线性核: K(x, y) = x^T * y, 是多项式核的特例  
多项式核: K(x, y) = (gamma x^T * y + b)^d  
高斯核:K(x_i, x_j) = e^(-gamma ‖x - y‖^2)  

对于数据x = (x_1, x_2), y = (y_1, y_2), 有多项式核 K(x, y) = (x^T * y)^2 = (x_1 * y_1 + x_2 * y_2)^2 = (x_1 * y_1)^2 + (x_2 * y_2)^2 + 2(x_1 * x_2 * y_1 * y_2) = 
phi(x) * phi(y)  
phi(x) = (x1^2, sqrt(2)x1x2, x_2^2), 因此二阶多项式核把x从二维映射到三维  
对于高斯核K(x_i, x_j) = e^(-gamma ‖x - y‖^2), 可以用泰勒展开式的方式拆开, 一共n项, n阶相当于把特征映射到n+1维, 所以高斯核是0~n阶多项式核的求和
...
c. 高维稀疏的ID类特征会使树模型的训练效率变得极为低效，且容易过拟合。树模型训练过程是一个贪婪选择特征的算法，要从候选特征集合中选择一个使分裂后信息增益最大的特征来分裂。按照高维的ID特征做分裂时，子树数量非常多，计算量会非常大，训练会非常慢; 同时，按ID分裂得到的子树的泛化性也比较弱，由于只包含了对应ID值的样本，样本稀疏时也很容易过拟合;
...
##### (66) n-gram https://zhuanlan.zhihu.com/p/32829048; word2vec: CBOW/skip-gram
...
##### (68) TF-IDF: term frequency–inverse document frequency, 词频-逆文档频率(log)
...
##### (71) CTR, CVR人群分析， x轴为点击率， y轴为转化率
1. 第一象限: 点击率高->转化率高: 精准的人群投放, 拓展优质优良， 扩大对于精准人群的曝光使其点击, 
2. 第二象限: 点击率低->转化率高: 糟糕的广告投放, 转化率高说明广告转化页本身没有问题, 但是点击率很低说明广告投放机制出现了问题; 优化排名以及优化广告创意, 提高点击率
3. 第三象限: 点击率低->转化率低: 不精准的人群投放, 既要优化广告创意/排名， 提高曝光度, 也要关注流量的匹配状况, 提高投放精准度
4. 第四象限: 点击率高->转化率低: 人群流失, 未发生转化, 优化产品的核心卖点, 提高广告落地页的创意度

##### (72) 广告主给了一批高转换人群包，问我们怎么用来提升业务
...
##### (82) Glove词向量: https://zhuanlan.zhihu.com/p/101179171
...
#### (87) k-means的分布式实现, map-reduce  
map: 将不同数据分配到不同的聚类中心, (x_i, center_j)  
shuffle: 将相同聚类中心的数据分配到一起  
reduce: 更新当前聚类中心下的数据, 计算新的聚类中心  

#### (88) 如何解决哈希冲突
开放定址法: 也叫再散列法, 发生哈希冲突时, 产生新的哈希地址, 包括线性探测, 二次探测, 伪随机探测, 缺点容易产生元素聚集  
再哈希法: 构造多个哈希函数, 一个冲突了换下一个, 直到不冲突为止, 不容易产生元素聚集, 但是增加了额外的计算量    
链地址法: 将所有哈希值相同的元素用链表链接起来, 原来哈希表的位置上存储链表的头指针, 因此查找、插入和删除主要在词链中进行, 适用于经常插入删除的情况  
...
GraphSage论文：邻居如何采样，节点如何更新，如何训练，可以进行有监督学习吗
算法题：排序数组中找目标的第一个位置
新闻推荐场景下有哪些评价指标，如何设计模型

Word2Vec负采样时的损失函数
BERT中NSP任务的缺点，ALBERT中如何改进
算法题：二维坐标内有N个点，不重合，两点可确定一条直线，求max|k|，k为直线的斜率；要求时间复杂度<O(N**2)
排序的代码


1.dfs与bfs的区别
2.找到数组topk大的最小值（面试题41）
3.二维矩阵的最短路径（1091. 二进制矩阵中的最短路径的变形，bfs+优先队列）
4.判断一棵树是不是BST（面试题 04.05. 合法二叉搜索树）
6.FM做召回的项目：如何训练FM模型
7.评论子句分类：重点说明类别不平衡问题如何处理
8.短视频内容理解与推荐竞赛：FM与FFM的区别及两者的时间复杂度；title Embedding为什么做mean pooling，为什么不用avg和max的方式
10.采样后的CTR值如何做矫正得到不采样的CTR，如原始100:1，采样后是10:1


1.接雨水的最大值（11.盛最多水的容器）
2.介绍堆、队列，堆pop的时间复杂度O(logn)
3.一维数组的max pooling(window_size=4, step=2, nums = [1, 8, 2, 3, 5, 1], res = [4, 5])(面试题59 - I. 滑动窗口的最大值)
5.召回项目：Word2Vec的使用方法；损失函数；高低频样本的处理；词向量是哪个矩阵，两个权重矩阵有什么区别；vw和vwi是来自同一个权重矩阵吗
6.FM召回项目：介绍FM和FFM的原理和区别
...
给你均值方差，让你利用正态分布随机生成1000个点（不能用库，说的不是很好）python random.py
