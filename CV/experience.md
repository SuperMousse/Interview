...
4.召回项目：如何确定是无淘用户；Faiss原理，用的ANN还是暴力搜索；用户在支付宝上的行为是否可以帮助召回，比如确定用户的消费等级；广告的付费方式是什么；如何让衡量模型的指标，CTR并不准确，如果是为了用户拉新的话，用户是否注册淘宝可以看作是一个转化，为什么不统计CVR呢；目的是做用户拉新吗
5.召回项目：Word2Vec的使用方法；损失函数；高低频样本的处理；词向量是哪个矩阵，两个权重矩阵有什么区别；vw和vwi是来自同一个权重矩阵吗
6.FM召回项目：介绍FM和FFM的原理和区别
7.部门了解：20-21人，博士一样、本科一人，工作过程中涉及全栈的工作，要扩招到25人左右
三面：
1.大根堆的定义和性质，堆如何存储，建堆的时间复杂度O（n）推导
2.看过哪些数据结构和机器学习的书，统计学系方法有实践过吗
3.树如何生成
4.有在Leetcode上刷题吗
5.Word2Vec召回，感觉比较简单，难点和可以改进的点，我的工作
6.比较想做什么，主要做广告，有完善的培养计划
